...
cql_test_config rest_authenticator_on(int port, bool use_outdated_certs) {
    ...
    cfg.db_config->rest_authenticator_endpoint_port(port);
    if (use_outdated_certs) {
        cfg.db_config->rest_authenticator_endpoint_cafile_path("tools/rest_authenticator_server/outdated_certs/ca.crt");
    } else {
        cfg.db_config->rest_authenticator_endpoint_cafile_path("tools/rest_authenticator_server/certs/ca.crt");
    }
...
class rest_authentication_handler : public seastar::httpd::handler_base {
    virtual future <std::unique_ptr<seastar::http::reply>>
    handle(const sstring &path, std::unique_ptr <seastar::http::request> req,
...
future<> with_dummy_authentication_server(std::function<void(cql_test_env&)> func, bool use_outdated_certs = false) {
    return seastar::async([func, use_outdated_certs] {
        int port = 54321; // TODO allocate a new port at each invocation to allow test execution in parallel
        auto conf = std::move(rest_authenticator_on(port, use_outdated_certs));
        ...
        if (use_outdated_certs) {
            b.set_x509_key_file("tools/rest_authenticator_server/outdated_certs/rest_api.crt",
                                "tools/rest_authenticator_server/outdated_certs/rest_api.key",
                                seastar::tls::x509_crt_format::PEM).get();
        } else {
            b.set_x509_key_file("tools/rest_authenticator_server/certs/rest_api.crt",
                                "tools/rest_authenticator_server/certs/rest_api.key",
                                seastar::tls::x509_crt_format::PEM).get();
        }
        ...
        httpd.listen(ipv4_addr("127.0.0.1", conf.db_config->rest_authenticator_endpoint_port())).get();
...
static future<> create_superuser_role(cql3::query_processor &qp) {
    ...
    return qp.execute_internal(
            ...
            {auth::meta::DEFAULT_SUPERUSER_NAME,
             auth::passwords::hash(sstring(auth::meta::DEFAULT_SUPERUSER_NAME), rng_for_salt)},
...
SEASTAR_TEST_CASE(rest_authenticator_conf) {
    return with_dummy_authentication_server([] (cql_test_env &env) {
        ...
        BOOST_REQUIRE_EQUAL(authenticator_config.rest_authenticator_endpoint_cafile_path,
                            "tools/rest_authenticator_server/certs/ca.crt");
...
SEASTAR_TEST_CASE(valid_user) {
    return with_dummy_authentication_server([] (cql_test_env &env) {
        ...
        BOOST_REQUIRE(rm.can_login("norole").get() == false);
    });
}
...
SEASTAR_TEST_CASE(expired_tls_certs) {
    return with_dummy_authentication_server([] (cql_test_env &env) {
        auto &a = env.local_auth_service().underlying_authenticator();
        ...
        BOOST_REQUIRE_EXCEPTION(a.authenticate(creds).get(), exceptions::authentication_exception,
            exception_predicate::message_contains("The certificate is NOT trusted. The certificate chain uses expired certificate."));
    }, true);
}
...
SEASTAR_TEST_CASE(unknown_user) {
    return with_dummy_authentication_server([] (cql_test_env &env) {
        ...
        // Check state through role_manager should be align with DB state
        auto &rm = env.local_auth_service().underlying_role_manager();
        BOOST_REQUIRE(rm.can_login("john.doe").get() == false);
    });
}
...
SEASTAR_TEST_CASE(invalid_credentials) {
    return with_dummy_authentication_server([] (cql_test_env &env) {
        ...
        BOOST_REQUIRE_EXCEPTION(a.authenticate(creds).get(), exceptions::authentication_exception,
                                exception_predicate::message_contains("Bad password"));
        ...
        // Check state through role_manager should be align with DB state
        auto &rm = env.local_auth_service().underlying_role_manager();
        BOOST_REQUIRE(rm.can_login("foo.bar").get() == false);
    });
}
...
SEASTAR_TEST_CASE(get_list_of_roles) {
    return with_dummy_authentication_server([] (cql_test_env &env) {
        ...
        auth::role_set roles;
        roles.insert(sstring("cassandra"));
