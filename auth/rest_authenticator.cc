...
logging::logger ralogger("rest_authenticator");
...
rest_authenticator::rest_authenticator(cql3::query_processor& qp, ::service::migration_manager& mm)
        ...
        , _credentials_builder(std::make_unique<tls::credentials_builder>())
    ...
    return default_role_row_satisfies(_qp, &has_salted_hash).then([this](bool exists) {
        if (!exists) {
            return _qp.execute_internal(
                ...
                ralogger.info("Created default superuser authentication record.");
            });
        }
...
future<> rest_authenticator::setup_reloadable_certs_credentials() {
    // Load CA
    if (_authenticator_config.rest_authenticator_endpoint_cafile_path != "") {
        co_await _credentials_builder->set_x509_trust_file(_authenticator_config.rest_authenticator_endpoint_cafile_path, tls::x509_crt_format::PEM);
    } else {
        co_await _credentials_builder->set_system_trust();
    }

    _certs = co_await _credentials_builder->build_reloadable_certificate_credentials([this](const std::unordered_set<sstring>& files, std::exception_ptr ep) {
        if (ep) {
            ralogger.warn("Exception while reloading {}: {}", files, ep);
        } else {
            ralogger.info("Reloaded {}", files);
        }
    });
}
...
future<> rest_authenticator::start() {
    ...
    return once_among_shards([this] {
        ...
        return setup_reloadable_certs_credentials();
    });
}
...
future<role_set> get_user_roles(sstring username, sstring password, const authenticator_config& conf, shared_ptr<seastar::tls::certificate_credentials> certs) {
    using namespace seastar::http;
    using namespace seastar::http::experimental;

    auto ip = co_await net::dns::resolve_name(conf.rest_authenticator_endpoint_host);
    ...
    auto bytes_authorization = bytes_view(reinterpret_cast<const signed char *>(authorization.c_str()), authorization.size());
    req._headers["Authorization"] = format("Basic {}", base64_encode(bytes_authorization));
    ...
    switch (rep._status)
    {
        case reply::status_type::ok: {
            ...
            for (auto it = groups_json->Begin(); it != groups_json->End(); ++it) {
                if (it->IsString()) {
                    groups.insert(sstring(rjson::to_string_view(*it)));
                }
            }
            co_return groups;
            break;
        ...
        case reply::status_type::unauthorized:
            throw exceptions::authentication_exception("Bad password");
            break;

        case reply::status_type::not_found:
            throw exceptions::authentication_exception("Unknown username");
            break;
...
future<authenticated_user> rest_authenticator::authenticate(
    ...
    if (!credentials.contains(USERNAME_KEY)) {
        ralogger.info("Required key 'USERNAME' is missing");
    ...
    if (!credentials.contains(PASSWORD_KEY)) {
        ralogger.info("Required key 'PASSWORD' is missing");
    ...
    return futurize_invoke([this, username, password] {
        ...
        if (username == DEFAULT_USER_NAME && (!salted_hash || !passwords::check(password, *salted_hash))) {
            throw exceptions::authentication_exception("Bad password for superuser");
            ...
            ralogger.info("Authenticating username {} from rest endpoint", username);
            auto f = get_user_roles(username, password, _authenticator_config, _certs);
            auto timeout = std::chrono::seconds(_authenticator_config.rest_authenticator_endpoint_timeout);
            // TODO manage retry?
            ...
            return with_timeout(timer<>::clock::now() + timeout, std::move(f))
    ...
    }).handle_exception([username] (std::exception_ptr ep) {
        try {
            try {
                std::rethrow_exception(ep);
            ...
            ralogger.warn("Exception while authenticating user {}: {}", username, std::current_exception());
            return current_exception_as_future<authenticated_user>();
        }
    });
}
...
rest_authenticator::create_or_update(bool create_user, sstring username, sstring password, role_set &roles) const {
    return do_with(std::move(roles), [this, create_user, username, password](role_set &roles) {
        ...
        if (create_user) {
            ralogger.info("Create role for username {}", username);
            return rest_authenticator::create_with_groups(username, roles, authen_options).then([username] {
                return make_ready_future<authenticated_user>(username);
            });
        }
        ralogger.info("Update password for username {}", username);
